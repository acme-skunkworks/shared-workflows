...
- **`required_status_checks`**, each **pinned to the GitHub Actions integration**
  ...
  - **`GO/NO GO`** — the per-repo aggregator check-run (see
    [the `GO/NO GO` gate](go-no-go-gate.md)). This is the single gate; it `needs:`
    every real CI job, so requiring it transitively requires them all.
...
check-run** (the aggregator from [the gate doc](go-no-go-gate.md), A-418). For
...
2. **Update it in place** with the versioned JSON (use the swapped file for
   `shared-workflows`):
   ...
   Or, if the repo has no repo-level ruleset yet, **create** one:
